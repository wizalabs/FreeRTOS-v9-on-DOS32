...
// @2019/3/11 by Ardi Wang for porting to DOS32
	...
	#pragma aux portSWITCH_CONTEXT =													\
			"mov	ebx, pxCurrentTCB"													\
			"mov	[ebx], esp"			/* Save the stack pointer into the TCB. */		\
			"call	vTaskSwitchContext"	/* Perform the switch. */						\
			"mov	ebx, pxCurrentTCB"	/* Restore the stack pointer from the TCB. */	\
			"mov	esp, [ebx]"															\
			"add	esp, 0x0004"														\
			"mov	ebp, esp"			/* Prepair the BP for restoration of the SP in ISR */

	#pragma aux portFIRST_CONTEXT =														\
			"mov	ebx, pxCurrentTCB"													\
			"mov	esp, [ebx]"		/* Remove the extra bytes that exist in	debug builds before restoring the context. */	\
			"add	esp, 0x0004"														\
			"pop	gs"																	\
			"pop	fs"																	\
			"pop	es"																	\
